...
class Settings:
    ...
    whisper_device: str = os.environ.get(
        "WHISPER_DEVICE",
        "cuda" if os.environ.get("USE_CUDA", "").lower() == "true" else "cpu",
    )
    whisper_compute_type: str = os.environ.get(
        "WHISPER_COMPUTE_TYPE",
        "float16" if os.environ.get("USE_CUDA", "").lower() == "true" else "int8",
    )

    block_seconds: int = int(os.environ.get("BLOCK_SECONDS", "210"))
    block_max_chars: int = int(os.environ.get("BLOCK_MAX_CHARS", "2600"))
    delete_audio_after_run: bool = os.environ.get("DELETE_AUDIO_AFTER_RUN", "true").lower() == "true"
